...
type RetryCallback func(statusCode int, body []byte, attempt int) (bool, time.Duration)
...
func (f *Forwarder) applyBackoffDelay(ctx context.Context, delay time.Duration, attempt int, req *http.Request) error {
	if delay <= 0 {
		return nil
	}
	f.log.Debug("Retrying request after delay", "attempt", attempt, "delay", delay, "requestID", req.Header.Get(constants.RequestIDHeader))
...
func (f *Forwarder) trySend(req *http.Request, requestMutator RequestMutator, attempt int, retryCallback RetryCallback) (bool, *http.Response, error) {
	...
	if err != nil {
		return false, nil, fmt.Errorf("forwarding request: %w", err)
	}

	// Check if we should retry based on status code
	if retryCallback == nil || resp.StatusCode < 400 {
	...
	resp.Body.Close()
	if readErr != nil {
		bodyBytes = fmt.Appendf(nil, "failed to read response body: %v", readErr)
	} else {
		resp.Body = io.NopCloser(bytes.NewReader(bodyBytes))
	}

	shouldRetry, delay := retryCallback(resp.StatusCode, bodyBytes, attempt)
	...
	if err := f.applyBackoffDelay(req.Context(), delay, attempt, req); err != nil {
		return false, nil, fmt.Errorf("request cancelled during backoff: %w", err)
	}
	return true, nil, nil
}
