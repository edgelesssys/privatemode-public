...
export class ChatRequest {
      ...
      async handleError (response) {
        ...
        // Combine user message with technical details
        const fullErrorMessage = userMessage + (errorMessage ? ` (${errorMessage})` : '')
        throw new Error(`${response.status} - ${fullErrorMessage}`)
      }
